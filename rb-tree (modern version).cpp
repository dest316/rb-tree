...
private:
	...
	void foundKeyOrder(NodePtr node, int key, bool& flag) {
		if (node != TNULL) {
			if (key == node->data)
			{
				flag = true;
	...
	void countOrder(NodePtr node, int& value, int& count){	//Изменённый метод preorder, который суммирует значения листьев и считает их количество
		if (node != TNULL)
		{
			value += node->data;
			count++;
	...
	double averageKeyValue(NodePtr node){ //Здесь считаем среднее арифметическое
		int value = 0;
		int count = 0;
		countOrder(root, value, count); //Точка входа в обход
		return ((double)value / count); //Возвращаем с.а.
	}
	...
	void deleteNodeHelper(NodePtr node, int key) {
		...
		if (z->left == TNULL) {
			...
			y = maximum(z->left); //было minimum(z->right)
...
public:
	...
	{
		cout << averageKeyValue(this->root) << endl; //Распечатываем среднее значение ключа в дереве
	}
